...
#ifndef SEED_LUASTATE_H
#define SEED_LUASTATE_H
...
#include "impl/LuaType.h"
	...
	public:
		...
		LuaState(const LuaState& that) = default;

		LuaState& operator = (const LuaState& that)	= default;

		lua_State* get_state() const { return  L; }

		friend bool operator == (const LuaState& lhs, const LuaState& rhs)
		{
			return lhs.L == rhs.L;
		}
		friend bool operator !=(const LuaState& lhs, const LuaState& rhs)
		...
		static  LuaState newState(lua_Alloc f, void *ud)
		{
		...
		lua_CFunction setPanicFunc(lua_CFunction panic) const
		{
		...
		// comparision and arithmetic functions
...
// get functions (Lua -> stack)
		...
		int getGlobal(const char* name) const
		{
		...
		{
			return lua_getmetatable(L, table_idx);
		}
